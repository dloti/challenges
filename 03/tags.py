...
def get_similarities(tags):
    ...
    matches = [tp for tp in product(tags, tags) if tp[0] != tp[1] and SequenceMatcher(None, tp[0],tp[1]).ratio() >= SIMILAR]
    return [tp for tp in matches if (tp[1], tp[0]) not in matches or len(tp[0])<len(tp[1])]
